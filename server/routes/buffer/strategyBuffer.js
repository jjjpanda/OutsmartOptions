...
module.exports = {
      ...
      if (user) {
        if (req.body.ticker != undefined && req.body.ticker != '') {
          Strategy.find({ user, ticker: req.body.ticker }).then((strategies) => {
            if (strategies) {
              res.json({ strategies: strategies.map((strat) => ({ ticker: strat.ticker, legs: strat.legs, name: strat.name })) });
            } else {
              res.json({ strategies: [] });
            }
          });
      ...
      if (user) {
        Strategy.findOne({ user, ticker: req.body.ticker, key: req.body.legs.reduce((key, leg) => `${key}${leg.symbol}${leg.quantity}${leg.isLong ? 'LONG' : 'SHORT'}`, '') }).then((strategy) => {
          if (strategy) {
            ...
            const newStrat = new Strategy({
              user, ticker: req.body.ticker, legs: req.body.legs, name: req.body.name, key: req.body.legs.reduce((key, leg) => `${key}${leg.symbol}${leg.quantity}${leg.isLong ? 'LONG' : 'SHORT'}`, ''),
            });
            newStrat.save().then(() => {
              res.json({ saved: true });
            });
  ...
  deleteStrategy(req, res) {
    User.findById(req.body.id).then((user) => {
      if (user) {
        Strategy.deleteOne({
          user, ticker: req.body.ticker, name: req.body.name, key: req.body.legs.reduce((key, leg) => `${key}${leg.symbol}${leg.quantity}${leg.isLong ? 'LONG' : 'SHORT'}`, ''),
        }).then((e) => {
          if (e.deletedCount == 1) {
